    ...
    if(email!="") {
        userDetails.findOne({email:email},(err,user)=>{
            ...
            if(!user) {
                ...
                let otp=genOTP();
                emailOTP(email,'<h4>Dear Sir/ Madam,</h4><h4>Please use the following OTP <'+otp+'> for changing the password.<p>for Inquries Contact: 9999XXXX87</p><p>Regards,</p><p>RPMS Team</p>').catch(console.error);
